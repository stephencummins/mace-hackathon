...
def add_bullets(
    ...
    *,
    ...
    Each item is either:
      - a plain string (single line bullet), or
      - a (headline, sub) tuple — headline bold, sub muted on the next line.
    """
    tb = slide.shapes.add_textbox(left, top, width, height)
    tf = tb.text_frame
    tf.word_wrap = True
...
def header_band(slide, title: str, subtitle: str | None = None):
...
def footer_band(slide, slide_num: int, total: int = 10):
    add_text(
        slide,
        Inches(0.5),
        SLIDE_H - Inches(0.45),
        Inches(6),
        Inches(0.3),
        "M+AI+CE Hackathon  ·  Mace Digital Compliance Checker",
        size=10,
        color=MUTED,
    )
    add_text(
